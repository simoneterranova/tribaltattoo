...
const images = shopConfig.disegni;
const N = images.length;

// ── Physics carousel constants ──────────────────────────────
const CARD_W           = 460;  // px — card intrinsic width
const CARD_H           = 600;  // px — card intrinsic height
const SPACING          = 500;  // px center-to-center
const DRAG_SENSITIVITY = 260;  // px of drag to advance exactly one card
const MAX_THROW        = 2;    // max slides per release flick

const LERP     = 0.10;
const SETTLE_E = 0.0005;

// ── Module-level physics state (disegni-specific, avoids collision with GallerySection) ──
const dph = {
  pos: 0, target: 0,
...
const dCardEls: (CardEl | null)[] = Array(N).fill(null);
const dDotEls:  (HTMLButtonElement | null)[] = Array(N).fill(null);
...
function dTick() {
  dph.pos += (dph.target - dph.pos) * LERP;
  dPaint();
  if (Math.abs(dph.target - dph.pos) < SETTLE_E) {
    dph.pos = dph.target; dPaint(); dph.raf = null; return;
  }
  dph.raf = requestAnimationFrame(dTick);
}

function dStartLoop() {
  if (dph.raf) cancelAnimationFrame(dph.raf);
  dph.raf = requestAnimationFrame(dTick);
}

function dStopAuto() { if (dph.timer) { clearInterval(dph.timer); dph.timer = null; } }
function dStartAuto() {
  dStopAuto();
  dph.timer = setInterval(() => { dph.target = Math.round(dph.pos) + 1; dStartLoop(); }, 3800);
}

function dGoTo(idx: number) {
  const cur    = Math.round(dph.pos);
  const curMod = ((cur % N) + N) % N;
  let   delta  = idx - curMod;
  if (delta >  N / 2) delta -= N;
  if (delta < -N / 2) delta += N;
  dph.target = cur + delta;
  dStartLoop(); dStartAuto();
}

function dStepBy(d: number) {
  dph.target = Math.round(dph.pos) + d;
  dStartLoop(); dStartAuto();
}

// ────────────────────────────────────────────────────────────
// Mobile Disegni — self-contained rAF + lerp carousel
// ────────────────────────────────────────────────────────────
const MOBILE_CARD_VISIBLE_W = 72; // vw — active card width
...
const MobileDisegni = () => {
  ...
  const mPaint = useCallback(() => {
    ...
    mCardRefs.current.forEach((el, i) => {
      ...
      const ov = el.querySelector<HTMLElement>(".dm-ov");
      const lb = el.querySelector<HTMLElement>(".dm-lb");
  ...
  return (
    <section id="disegni" className="border-t-2 border-accent/20 py-16 px-6">
      <ScrollReveal direction="up" duration={0.7}>
        <div className="mb-8">
          <span className="font-body text-xs tracking-[0.3em] md:tracking-[0.4em] text-accent uppercase">
            {shopConfig.sections.disegni.label}
          </span>
          <h2 className="font-heading text-4xl md:text-6xl lg:text-8xl text-foreground mt-2 leading-none">
            {Array.isArray(shopConfig.sections.disegni.heading) ? (
              <>
                {shopConfig.sections.disegni.heading[0]}<br />
                {shopConfig.sections.disegni.heading[1]}<span className="text-primary">.</span>
              </>
            ) : (
              <>{shopConfig.sections.disegni.heading}<span className="text-primary">.</span></>
            )}
          </h2>
        </div>
        <div className="mb-6">
...
// ────────────────────────────────────────────────────────────
// Desktop Disegni — physics-based 3D carousel
// ────────────────────────────────────────────────────────────
const DesktopDisegni = () => {
  const stageRef = useRef<HTMLDivElement>(null);

  // Reset physics on mount to avoid leftover state from HMR / prior sessions
  useEffect(() => {
    dph.pos = 0; dph.target = 0; dph.down = false;
  }, []);
  ...
  useEffect(() => {
    ...
    const onDown = (x: number) => {
      dStopAuto();
      if (dph.raf) { cancelAnimationFrame(dph.raf); dph.raf = null; }
      dph.down = true;
      dph.x0 = x; dph.pos0 = dph.pos;
      dph.lx = x; dph.lt = performance.now(); dph.iv = 0;
    };
    ...
    const onMove = (x: number) => {
      ...
      const dt  = now - dph.lt;
      if (dt > 0) {
        const raw = (x - dph.lx) / dt / DRAG_SENSITIVITY;
        dph.iv = dph.iv * 0.70 + raw * 0.30;
      }
      dph.lx = x; dph.lt = now;
      dph.pos    = dph.pos0 - (x - dph.x0) / DRAG_SENSITIVITY;
      dph.target = dph.pos;
      if (!dph.raf) dph.raf = requestAnimationFrame(() => { dph.raf = null; dPaint(); });
    };
    ...
    const onUp = () => {
      ...
      dph.down = false;
      if (dph.raf) { cancelAnimationFrame(dph.raf); dph.raf = null; }
      const base    = Math.round(dph.pos);
      const thrown  = Math.round(dph.pos - dph.iv * 120);
    ...
    const tm = (e: TouchEvent) => {
      if (!dph.down) return;
      if (Math.abs(e.touches[0].clientX - dph.x0) > 6) e.preventDefault();
      onMove(e.touches[0].clientX);
    };
    const te = () => onUp();
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft")  dStepBy(-1);
      if (e.key === "ArrowRight") dStepBy(1);
    };
  ...
  return (
    <section id="disegni" className="py-20 border-t-2 border-accent/20">
      {/* Header */}
        ...
        >
          ...
          {images.map((img, i) => (
            <div
              ...
              ref={(el) => {
                if (!el) return;
                dCardEls[i] = el as CardEl;
                (dCardEls[i] as CardEl)._overlay = el.querySelector<HTMLElement>(".dc-ov");
                (dCardEls[i] as CardEl)._label   = el.querySelector<HTMLElement>(".dc-lb");
              }}
              onClick={() => {
                if (!dph.down) {
                  const ai = ((Math.round(dph.pos) % N) + N) % N;
                  if (i !== ai) dGoTo(i);
                }
              }}
              style={{
...
const DisegniSection = () => {
  ...
  return isMobile ? <MobileDisegni /> : <DesktopDisegni />;
};
