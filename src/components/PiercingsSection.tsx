...
const images = shopConfig.piercings;
...
// ── Autoplay ─────────────────────────────────────────────────
function stopAuto() {
  if (ph.timer) { clearInterval(ph.timer); ph.timer = null; }
}
function startAuto() {
  stopAuto();
  ph.timer = setInterval(() => {
    ph.target = Math.round(ph.pos) + 1;
    startLoop();
  }, 4500);
}
...
const MOBILE_LERP           = 0.10;
const MOBILE_SETTLE_E       = 0.001;
...
const MobileGallery = () => {
  ...
  const mPaint = useCallback(() => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (abs > 2.5) { el.style.visibility = "hidden"; continue; }
      el.style.visibility = "";

      const vwPx = window.innerWidth / 100;
      const cardW = MOBILE_CARD_VISIBLE_W * vwPx;
      const spacing = cardW * 1.12;

      const tx = d * spacing;
      const scale = Math.max(0.75, 1 - abs * 0.10);
      const op    = Math.max(0.25, 1 - abs * 0.25);
      const zi    = String((15 - abs * 3) | 0);
  ...
  const mTick = useCallback(() => {
    const s = mState.current;
    s.pos += (s.target - s.pos) * MOBILE_LERP;
    mPaint();
    if (Math.abs(s.target - s.pos) < MOBILE_SETTLE_E) {
      s.pos = s.target; 
      mPaint(); 
      s.raf = null; 
  ...
  useEffect(() => {
    ...
    const s = mState.current;
    s.pos = 0; s.target = 0; s.down = false;
    ...
    const onMove = (x: number) => {
      if (!s.down) return;
      const dx   = x - s.x0;
      const sens = MOBILE_CARD_VISIBLE_W * vwPx * 1.12;
      s.pos = s.pos0 - dx / sens;

      const now = performance.now();
      const dt  = now - s.lt;
      if (dt > 0) s.iv = (x - s.lx) / dt * 16;
      s.lx = x;
      s.lt = now;
  ...
  }, [mPaint, mStartAuto, mStartLoop, mStopAuto]);

  // Keyboard navigation
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      const s = mState.current;
      if (e.key === "ArrowLeft")  { s.target = Math.round(s.pos) - 1; mStartLoop(); mStartAuto(); }
      if (e.key === "ArrowRight") { s.target = Math.round(s.pos) + 1; mStartLoop(); mStartAuto(); }
    };
  ...
  return (
    <section id="piercings" className="border-t border-border py-16 px-6">
      <ScrollReveal direction="up" duration={0.7}>
        ...
        <div className="mb-6">
          <Link to="/piercings" title="Guarda la gallery completa dei piercings">
...
const DesktopGallery = () => {
  ...
  useEffect(() => {
    ...
    const onMove = (x: number) => {
      if (!ph.down) return;
      const dx = x - ph.x0;
      ph.pos = ph.pos0 - dx / DRAG_SENSITIVITY;

      const now = performance.now();
      const dt  = now - ph.lt;
      if (dt > 0) ph.iv = (x - ph.lx) / dt * 16;
      ph.lx = x;
      ph.lt = now;
    ...
    stage.addEventListener("mousedown",     md, { passive: false });
    window.addEventListener("mousemove", mm);
    window.addEventListener("mouseup",   mu);
  ...
  return (
    <section id="piercings" className="border-t border-border py-20">
      {/* Header */}
